...
LOG_FILE="$GLOBEX_DIR/ralph-wiggum.log"
MAX_ITERATIONS=100
...
usage() {
  cat << 'EOF'
Ralph-Wiggum Loop for Globex - Implementation with Validation

USAGE:
  ./scripts/ralph-wiggum-loop.sh [OPTIONS]
...
DESCRIPTION:
  Two-agent loop:
  1. Ralph: Implements ONE feature, outputs <ralph>DONE:FEATURE_ID</ralph>
  2. Wiggum: Validates implementation, outputs <wiggum>APPROVED</wiggum> or <wiggum>REJECTED:reason</wiggum>
  
  On rejection, Ralph retries with feedback. Loop continues until ALL_FEATURES_COMPLETE.
...
# Create .globex directory if missing
mkdir -p "$GLOBEX_DIR"
...
if [[ ! -f "$GLOBEX_DIR/state.json" ]]; then
  log "ERROR: .globex/state.json not found. Run /globex-init first."
  exit 1
fi

CURRENT_PHASE=$(jq -r '.currentPhase' "$GLOBEX_DIR/state.json")
...
while [[ $ITERATION -le $MAX_ITERATIONS ]]; do
  ...
  WIGGUM_PROMPT="Validate feature $FEATURE_ID implementation. Check acceptance criteria in .globex/features.json. Output <wiggum>APPROVED</wiggum> if all criteria pass, or <wiggum>REJECTED:reason</wiggum> with specific feedback."
